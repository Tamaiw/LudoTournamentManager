...
export function DashboardPage() {
  ...
  const nextEvent = liveTournament
    ? {
        id: liveTournament.id,
        name: liveTournament.name,
        type: 'tournament' as const,
        status: liveTournament.status,
      }
    : liveLeague
      ? {
          id: liveLeague.id,
          name: liveLeague.name,
          type: 'league' as const,
          status: liveLeague.status,
        }
      : null
            ...
            {recentLeagues.map((l) => (
              <Card key={l.id}>
                <div className="flex items-center justify-between">
                  ...
                  <Link to={`/leagues/${l.id}`} className="text-blue-600 hover:underline text-sm">
                    View
                  </Link>
                </div>
              </Card>
